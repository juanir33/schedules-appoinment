...
export interface ServiceMetrics {
  mostPopular: { serviceId: string; serviceName: string; count: number; revenue: number }[];
  leastPopular: { serviceId: string; serviceName: string; count: number; revenue: number }[];
  byService: { serviceId: string; serviceName: string; count: number; revenue: number }[];
}
 ...
 */
export async function getReservationMetrics(dateRange: AnalyticsDateRange): Promise<ReservationMetrics> {
 ...
 */
export async function getRevenueMetrics(dateRange: AnalyticsDateRange): Promise<RevenueMetrics> {
  const reservationsRef = adminDb.collection('reservations');
  ...
  const revenueData = reservations.map(reservation => {
    ...
    if ([ReservationStatus.CONFIRMED, ReservationStatus.PENDING, ReservationStatus.COMPLETED].includes(reservation.status)) {
  ...
  const completedReservations = revenueData.filter(r => r.status === ReservationStatus.COMPLETED);
  const averageTicket = completedReservations.length > 0 ? totalRevenue / completedReservations.length : 0;
 ...
 */
export async function getServiceMetrics(dateRange: AnalyticsDateRange): Promise<ServiceMetrics> {
  const reservationsRef = adminDb.collection('reservations');
  const servicesRef = adminDb.collection('services');
  ...
  const serviceStats = new Map<string, { serviceId: string; serviceName: string; count: number; revenue: number }>();
  ...
  const mostPopular = [...byService].sort((a, b) => b.count - a.count).slice(0, 5);
  const leastPopular = [...byService].sort((a, b) => a.count - b.count).slice(0, 5);
 ...
 */
export async function getTimeSlotMetrics(dateRange: AnalyticsDateRange): Promise<TimeSlotMetrics> {
  const reservationsRef = adminDb.collection('reservations');
 ...
 */
export async function getCancellationMetrics(dateRange: AnalyticsDateRange): Promise<CancellationMetrics> {
  const reservationsRef = adminDb.collection('reservations');
